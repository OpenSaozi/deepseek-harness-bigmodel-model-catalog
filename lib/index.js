import { InMemoryCredentialStore, defaultProviderAuthContext } from "@earendil-works/pi-ai";
...
function resolveConfig(config) {
	...
	if (!Number.isInteger(maxRequestImageBytes) || maxRequestImageBytes <= 0) throw new Error("model-catalog-bigmodel: maxRequestImageBytes must be a positive integer");
	if (!Number.isSafeInteger(requestImagePixelBudget) || requestImagePixelBudget <= 0) throw new Error("model-catalog-bigmodel: requestImagePixelBudget must be a positive safe integer");
	if (!Number.isSafeInteger(requestImageMaxBytes) || requestImageMaxBytes <= 0) throw new Error("model-catalog-bigmodel: requestImageMaxBytes must be a positive safe integer");
	return {
